...
//   Y(0) = std::log(mu);
//   S(0, 0) = s;
//   out = as_scalar(log_normpdf(X, Y, S));
//   return out;
// };
// 
...
// 
// double logWishartProbability(arma::mat X, arma::mat V, double n, arma::uword P){
//   double out = 0.5*((n - P - 1) * log_det(X).real() - trace(inv_sympd(V) * X) - n * log_det(V).real());
//   return out;
// }
// 
// 
// double logInverseWishartProbability(arma::mat X, arma::mat Psi, double nu, arma::uword P){
//   return -0.5*(nu*log_det(Psi).real()+(nu+P+1)*log_det(X).real()+trace(Psi * inv_sympd(X)));
// }
// 
// class sampler {
// 
...
//   arma::vec concentration, ll, likelihood; // w,
//   arma::umat members;
...
//   // };
// 
//   virtual void updateAllocation() = 0; // {
//   //
//   //   double u = 0.0;
//   //   uvec uniqueK;
//   //   vec comp_prob(K);
...
//   virtual arma::vec itemLogLikelihood(arma::vec x, arma::uword b) = 0 ; // { return arma::vec(); };
// 
// };
// 
// 
// class samplerWithGlobalWeights : public virtual sampler {
// 
// private:
// 
// public:
...
// 
//   // Destructor
//   virtual ~samplerWithGlobalWeights() { };
// 
//   // Functions required of all mixture models
...
// 
// 
// class samplerWithBatchWeights: virtual public sampler {
// 
// private:
// 
// public:
...
// 
//   // Destructor
//   virtual ~samplerWithBatchWeights() { };
// 
//   // Functions required of all mixture models
//   void updateWeights(){
// 
...
// 
// 
// class semisupervisedSamplerWithBatchWeights :
//   public virtual samplerWithBatchWeights {
// private:
// 
// public:
// 
...
//   // arma::mat alloc_prob;
// 
//   using samplerWithBatchWeights::samplerWithBatchWeights;
// 
//   semisupervisedSamplerWithBatchWeights(
//     arma::uword _K,
//     arma::uword _B,
//     arma::uvec _labels,
//     arma::uvec _batch_vec,
...
//     arma::uvec _fixed
//   ) :
//     samplerWithBatchWeights(_K, _B, _labels, _batch_vec, _concentration, _X),
...
// 
//   // Destructor
//   virtual ~semisupervisedSamplerWithBatchWeights() { };
// 
//   virtual void updateAllocation() {
// 
//     uword b = 0;
...
// public:
// 
//   uword n_param_cluster = 1 + P + P * (P + 1) * 0.5,
//     n_param_batch = 2 * P;
// 
...
//   vec xi, cov_log_det, global_mean;
//   mat scale, mu, m, S, phi, cov_comb_log_det, mean_sum, global_cov, Y;
//   cube cov, cov_inv, cov_comb, cov_comb_inv;
// 
//   using sampler::sampler;
// 
//   mvnSampler(
...
//     xi = mean_mat.col(0);
// 
//     // mu_upper = xi + 10 * abs(xi - X_max.t());
//     // mu_lower = xi - 10 * abs(xi - X_min.t());
// 
//     // std::cout << "\n\nMu upper:\n" << mu_upper << "\n\nMu lower:\n" << mu_lower;
// 
...
//     // std::cout << "\nPrior scale:\n" << scale;
// 
//     // scale = inv( cov(X) / std::pow(K, 2.0 / P) );
//     // scale = cov(X) / std::pow(K, 2.0 / P);
// 
//     // std::cout << "\n\nPrior scale:\n" << scale;
// 
...
//     //   mu.col(k) = mvnrnd(xi, (1.0/kappa) * cov.slice(k), 1);
//     //
//     //   // while(all(mu.col(k) < mu_lower) || all(mu.col(k) > mu_upper) ) {
//     //   //   mu.col(k) = mvnrnd(xi, (1.0/kappa) * cov.slice(k), 1);
//     //   // }
//     //
//     //
...
//     //     //   m(p, b) = 0.0;
//     //     // } else {
//     //         S(p, b) = 1.0; // S_loc + 1.0 / randg<double>( distr_param(rho, 1.0 / theta ) );
//     //     // S(p, b) = 1.0;
//     //       m(p, b) = randn<double>() * S(p, b) / lambda + delta(p);
//     //     // m(p, b) = randn<double>() / lambda + delta(p);
//     //     // }
//     //   }
//     // }
// 
...
//       for(uword b = 0; b < B; b++) {
//         cov_comb.slice(k * B + b) = cov.slice(k); // + diagmat(S.col(b))
//         for(uword p = 0; p < P; p++) {
...
//     //   score +=  as_scalar(cov_comb_log_det(k, b) + dist_from_mean.t() * cov_comb_inv.slice(k * B + b) * dist_from_mean);
//     // }
//     //
...
//     //   // std::cout << "\nThe invariance.";
//     //
//     //   my_inv = inv_sympd(my_cov_comb);
//     //
//     //   // std::cout << "\nDeterminant.";
//     //   my_det = log_det(my_cov_comb).real();
//     //
//     //   dist_from_mean = X_t.col(n) - mu.col(k) - m.col(b);
//     //   score_alt += -0.5 * as_scalar(my_det + (dist_from_mean.t() * my_inv * dist_from_mean));
//     // }
// 
...
//     //   score += as_scalar(cov_comb_log_det(b) + (dist_from_mean.t() * cov_comb_inv.slice(b) * dist_from_mean));
//     // }
//     //
//     // score += as_scalar((nu + P + 2) * cov_log_det + kappa * ((mu.col(k) - xi).t() * cov_inv * (mu.col(k) - xi)) + trace(scale * cov_inv));
//     // score *= -0.5;
//     //
//     // if(std::abs(score - score_alt) > 1e-6 ) {
//     //   std::cout << "\n\nOriginal: " << score << "\nMy new: " << score_alt;
...
//       for(uword p = 0; p < P; p++) {
// 
//         // (randn() * m_proposal_window) + m(p, b);
// 
//         // S_proposed(p) = S_loc + (S(p, b) - S_loc) * std::exp(randn() * S_proposal_window);
//         // S_proposed(p) = S_loc + std::exp(randn() * S_proposal_window + log(S(p, b) - S_loc) );
...
//         // proposed_cov_comb_inv.slice(k) = inv(proposed_cov_comb.slice(k));
...
// 
// 
// class semisupervisedSamplerWithGlobalWeights : public virtual samplerWithGlobalWeights {
// private:
// 
// public:
// 
...
//   // arma::mat alloc_prob;
// 
//   using samplerWithGlobalWeights::samplerWithGlobalWeights;
// 
//   semisupervisedSamplerWithGlobalWeights(
//     arma::uword _K,
//     arma::uword _B,
//     arma::uvec _labels,
//     arma::uvec _batch_vec,
...
//     arma::uvec _fixed
//   ) :
//     samplerWithGlobalWeights(_K, _B, _labels, _batch_vec, _concentration, _X),
//     sampler(_K, _B, _labels, _batch_vec, _concentration, _X) {
// 
//     arma::uvec fixed_ind(N);
// 
...
// 
//   // Destructor
//   virtual ~semisupervisedSamplerWithGlobalWeights() { };
// 
//   virtual void updateAllocation() {
// 
//     double u = 0.0;
...
// 
// class mvnPredictive :
//   public virtual mvnSampler,
//   public virtual semisupervisedSamplerWithGlobalWeights {
// 
// private:
// 
// public:
...
//   using semisupervisedSamplerWithGlobalWeights::semisupervisedSamplerWithGlobalWeights;
// 
//   mvnPredictive(
//     arma::uword _K,
//     arma::uword _B,
...
//       _concentration,
//       _X),
//       semisupervisedSamplerWithGlobalWeights(_K, _B, _labels, _batch_vec, _concentration, _X, _fixed)
//     {
//   };
// 
...
// 
// 
// class mvnPredictiveWithBatchWeights :
//   public virtual mvnSampler,
//   public semisupervisedSamplerWithBatchWeights {
// 
// private:
// 
// public:
// 
//   // using mvnSampler::mvnSampler;
//   using semisupervisedSamplerWithBatchWeights::semisupervisedSamplerWithBatchWeights;
// 
//   mvnPredictiveWithBatchWeights(
//     arma::uword _K,
//     arma::uword _B,
//     double _mu_proposal_window,
//     double _cov_proposal_window,
...
//     sampler(_K, _B, _labels, _batch_vec, _concentration, _X),
//     samplerWithBatchWeights(_K, _B, _labels, _batch_vec, _concentration, _X),
//     semisupervisedSamplerWithBatchWeights(_K, _B, _labels, _batch_vec, _concentration, _X, _fixed),
//     mvnSampler(_K,
//       _B,
//       _mu_proposal_window,
//       _cov_proposal_window,
...
//   };
// 
//   virtual ~mvnPredictiveWithBatchWeights() { };
// };
// 
// class mvnFactory {
// public:
...
//   };
// 
//   static std::unique_ptr<mvnSampler> createSampler(weightsType type,
//     arma::uword K,
//     arma::uword B,
...
// 
//     case batch_weights :
//       std::cout << "\n\nBatch weights in use";
//       return std::make_unique<mvnPredictiveWithBatchWeights>(K,
//         B,
//         mu_proposal_window,
//         cov_proposal_window,
//         m_proposal_window,
...
// };
// 
// class msnPredictive : public msnSampler, public semisupervisedSamplerWithGlobalWeights {
// 
// private:
// 
// public:
...
// 
// 
// class mvtPredictiveBatchWeights : public mvtSampler,
//                       public semisupervisedSamplerWithBatchWeights {
// 
// private:
// 
...
//   };
// 
//   virtual ~mvtPredictiveBatchWeights() { };
// 
// };
// 
// class mvtFactory {
...
//   };
// 
//   static std::unique_ptr<mvtSampler> createSampler(weightsType type,
//                                                 arma::uword K,
//                                                 arma::uword B,
//                                                 double mu_proposal_window,
//                                                 double cov_proposal_window,
...
// 
//     case batch_weights :
//       return std::make_unique<mvtPredictiveBatchWeights>(K,
//                                                          B,
//                                                          mu_proposal_window,
//                                                          cov_proposal_window,
//                                                          m_proposal_window,
...
// //   };
// //
// //   static std::unique_ptr<semisupervisedSamplerWithGlobalWeights> createSemisupervisedSampler(samplerType type,
// //     arma::uword K,
// //     arma::uword B,
// //     double mu_proposal_window,
// //     double cov_proposal_window,
...
// 
// 
// //' @title Sample semi-supervised mixture of MVNs
...
//   //
//   // Declare the factory
//   mvnFactory my_factory;
// 
//   // Convert from an int to the samplerType variable for our Factory
//   mvnFactory::weightsType val = static_cast<mvnFactory::weightsType>(weightType);
// 
//   // Make a pointer to the correct type of sampler
//   std::unique_ptr<mvnSampler> sampler_ptr = my_factory.createSampler(val,
//     K,
//     B,
//     mu_proposal_window,
//     cov_proposal_window,
...
//   mat weights_saved_0 = zeros<mat>(floor(R / thin), K);
// 
...
//     weights_saved_1(K, B, floor(R / thin));
// 
...
// 
//   sampler_ptr->sampleFromPriors();
//   sampler_ptr->matrixCombinations();
// 
//   // Iterate over MCMC moves
//   for(uword r = 0; r < R; r++){
//     std::cout << "\n\nUpdate weights.";
...
//       sampler_ptr->updateBatchCorrectedData();
// 
// 
//       // Save all the objects of interest
//       // BIC_record( save_int ) = my_sampler.BIC;
//       // observed_likelihood( save_int ) = my_sampler.observed_likelihood;
//       // class_record.row( save_int ) = my_sampler.labels.t();
//       // acceptance_vec( save_int ) = my_sampler.accepted;
...
//       //
//       // if(weightType == 0) {
//       //   weights_saved_0.row( save_int ) = my_sampler.w.t();
//       // } else {
//       //   weights_saved_1.slice( save_int ) = my_sampler.w;
//       // }
// 
//       BIC_record( save_int ) = sampler_ptr->BIC;
//       observed_likelihood( save_int ) = sampler_ptr->observed_likelihood;
//       class_record.row( save_int ) = sampler_ptr->labels.t();
//       acceptance_vec( save_int ) = sampler_ptr->accepted;
//       mu_saved.slice( save_int ) = sampler_ptr->mu;
//       m_saved.slice( save_int ) = sampler_ptr->m;
//       S_saved.slice( save_int ) = sampler_ptr->S;
//       mean_sum_saved.slice( save_int ) = sampler_ptr->mean_sum;
// 
//       alloc_prob.slice( save_int ) = sampler_ptr->alloc;
//       cov_saved.slice ( save_int ) = reshape(mat(sampler_ptr->cov.memptr(), sampler_ptr->cov.n_elem, 1, false), P, P * K);
//       cov_comb_saved.slice( save_int) = reshape(mat(sampler_ptr->cov_comb.memptr(), sampler_ptr->cov_comb.n_elem, 1, false), P, P * K * B);
// 
//       batch_corrected_data.slice( save_int ) =  sampler_ptr->Y;
// 
//       complete_likelihood( save_int ) = sampler_ptr->complete_likelihood;
// 
//       std::cout << "\n\nSaving weights.\nWeights:\n" << sampler_ptr->w;
//       if(weightType == 0) {
//         weights_saved_0.row( save_int ) = sampler_ptr->w.t();
//       } else {
//         weights_saved_1.slice( save_int ) = sampler_ptr->w;
//       }
// 
//       // if(printCovariance) {
//       //   std::cout << "\n\nCovariance cube:\n" << my_sampler.cov;
...
//         Named("cov_acceptance_rate") = conv_to< vec >::from(sampler_ptr->cov_count) / R,
//         Named("mu_acceptance_rate") = conv_to< vec >::from(sampler_ptr->mu_count) / R,
//         Named("S_acceptance_rate") = conv_to< vec >::from(sampler_ptr->S_count) / R,
//         Named("m_acceptance_rate") = conv_to< vec >::from(sampler_ptr->m_count) / R,
...
// //       t_df_saved.row( save_int ) = my_sampler.t_df.t();
// //
// //       alloc_prob.slice( save_int ) = my_sampler.alloc;
// //       cov_saved.slice ( save_int ) = reshape(mat(my_sampler.cov.memptr(), my_sampler.cov.n_elem, 1, false), P, P * K);
// //       cov_comb_saved.slice( save_int) = reshape(mat(my_sampler.cov_comb.memptr(), my_sampler.cov_comb.n_elem, 1, false), P, P * K * B);
// //
// //       my_sampler.updateBatchCorrectedData();
