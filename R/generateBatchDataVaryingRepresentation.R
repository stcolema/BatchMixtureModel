...
generateBatchDataVaryingRepresentation <- function(N,
  ...
  for (b in seq(1, B)) {
    ...
    labels[batch_ind] <- sample(1:K, N_b, replace = T, prob = group_weights[, b])
  }
  ...
  for (p in seq(1, P)) {
    ...
    for (n in seq(1, N)) {
      ...
      true_data[n, p] <- x * reordered_std_devs[k] + reordered_group_means[k]
      observed_data[n, p] <- (x * reordered_std_devs[k] * reordered_batch_var[b]
        + reordered_group_means[k] + reordered_batch_shift[b])
    }
  }
